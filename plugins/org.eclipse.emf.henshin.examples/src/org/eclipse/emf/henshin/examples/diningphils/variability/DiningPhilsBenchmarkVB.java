 ...
 * </copyright>
 */
package org.eclipse.emf.henshin.examples.diningphils.variability;
 ...
 */
public class DiningPhilsBenchmarkVB {

	/**
	 * Relative path to the example files.
	 */
	public static final String PATH = "src/org/eclipse/emf/henshin/examples/diningphils/variability";
	 ...
	 */
	public static void run(String path, int numThreads) {
		...
		StateSpace stateSpace = resourceSet.getStateSpace("5-phils_VB.henshin_statespace");
...
//			int expectedStates = 22;
//			if (stateSpace.getStateCount() != expectedStates || !stateSpace.getOpenStates().isEmpty()) {
//				throw new StateSpaceException("Unexpected number of states: " + stateSpace.getStateCount() + "("
//						+ stateSpace.getOpenStates().size() + ")");
//			}

			// Print benchmark info:
			System.out.println(
					phils + "\t" + stateSpace.getStateCount() + "\t" + stateSpace.getTransitionCount() + "\t" + time);

		} catch (

		StateSpaceException e) {
			e.printStackTrace();
		} finally {
			manager.shutdown();
		}
