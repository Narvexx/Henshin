...
public class BasicStateSpaceManager extends StateSpaceIndexImpl implements StateSpaceManager {
	...
	/**
	 * Flag indicating whether duplicate transitions with the same label should be
	 * ignored.
	 */
	...
	protected void refreshHelpers() {
		...
		ignoreDuplicateTransitions = (ign != null)
				&& (ign.trim().equalsIgnoreCase("true") || ign.trim().equalsIgnoreCase("yes"));
	...
	protected boolean isOpen(State state) throws StateSpaceException {
		...
		for (Transition current : transitions) {
			...
			Transition transition = state.getOutgoing(target, current.getRule(), current.getMatch(),
	...
	/**
	 * Create a new open state in the state space. Warning: this does not check if
	 * an equivalent state exists already or whether the hash code is incorrect.
	 * 
	 ...
	 * 
	 * @see
	 * org.eclipse.emf.henshin.statespace.StateSpaceManager#createInitialState(org.
	 ...
	 * 
	 * @see
	 * org.eclipse.emf.henshin.statespace.StateSpaceManager#removeState(org.eclipse.
	 * emf.henshin.statespace.State)
	 */
	 ...
	 * 
	 * @see
	 * org.eclipse.emf.henshin.statespace.StateSpaceManager#mergeTerminalState()
	 */
	@Override
	 ...
	 * 
	 * @see
	 * org.eclipse.emf.henshin.statespace.StateSpaceManager#resetStateSpace(boolean)
	 */
	...
	protected State findState(Model model, int hashCode, Collection<State> states) throws StateSpaceException {
		for (State state : states) {
			if (hashCode == state.getHashCode() && getStateSpace().getEqualityHelper().equals(model, getModel(state))) {
				return state;
			}
		}
		return null;
	...
	/**
	 * Get the distance of a state to an initial state. This returns the correct
	 * distance only if the maximum state distance of the state space is positive.
	 ...
	 * 
	 * @see
	 * org.eclipse.emf.henshin.statespace.impl.StateSpaceIndexImpl#deriveModel(org.
	 * eclipse.emf.henshin.statespace.State, boolean)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * org.eclipse.emf.henshin.statespace.StateSpaceManager#exploreStates(java.util.
	 * List, boolean)
	 */
...
//				EGraph graph = state.getModel().getEGraph();
//				for (EObject eo : graph) {
//					EList<EAttribute> attrs = eo.eClass().getEAllAttributes();
//					System.out.println(eo.eClass().getName());
//					for (EAttribute a : attrs) {
//						System.out.println('\t' + a.getName() + " = " + eo.eGet(a));
//					}
//					Map<EObject, String> references = new HashMap<EObject, String>();
//					for (FeatureIterator<EObject> iterator = (FeatureIterator<EObject>) eo.eCrossReferences()
//							.iterator(); iterator.hasNext();) {
//						EObject eCrossReferencedObject = iterator.next();
//						if (iterator.feature().getName().contentEquals("holds")) {
//							System.out.println("\tholding = " + references.get(eCrossReferencedObject) + " " + eCrossReferencedObject.toString());
//						}
//						references.put(eCrossReferencedObject, iterator.feature().getName());
//					}
//					
//				}
			}
