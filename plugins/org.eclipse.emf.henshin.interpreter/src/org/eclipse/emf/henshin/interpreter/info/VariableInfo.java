...
public class VariableInfo {
	...
	public VariableInfo(RuleInfo ruleInfo, EngineImpl engine) {
		...
		FeatureConfiguration.INSTANCE.initializeConfigurations(VariabilityFactory.INSTANCE.createVariabilityRule(rule));
	...
	private void createConstraints(Node node) {
		...
		for (Edge edge : node.getOutgoing()) {
			...
			target = variable2mainVariable.get(target);
			...
			if (index != null && index.length() > 0) {
				...
				String pcString = VariabilityFactory.INSTANCE.createVariabilityEdge(edge).getPresenceCondition();
				Sentence presenceCondition = pcString.length() > 0 ? FeatureExpression.getExpr(pcString) : null;
				constraint = new ReferenceConstraint(target, edge.getType(), presenceCondition, null, true);
			}
