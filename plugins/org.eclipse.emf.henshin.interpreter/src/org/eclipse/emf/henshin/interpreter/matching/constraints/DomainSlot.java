...
public class DomainSlot implements Cloneable, Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = -1484231269027653941L;
	...
	/**
	 * Flag that describes whether this domain slot is locked. A slot is locked if a
	 * value from the domain is selected that fulfills the constraints of the owner
	 * variable.
	 */
	boolean locked;
	...
	/**
	 * All changes done to other domain slots by ReferenceConstraints of variables
	 * that use this domain slot.
	 */
	final Map<BinaryConstraint, DomainChange> remoteChangeMap;

	/**
	 * A collection of parameters that were initialized by constraints belonging to
	 * variables of this domain slot.
	 */
	...
	/**
	 * A collection of variables whose constraints were already validated against
	 * the current value.
	 */
	 ...
	 */
	public DomainSlot(ConditionHandler conditionHandler, Set<EObject> usedObjects, boolean injective, boolean dangling,
			boolean deterministic, boolean inverseMatchingOrder, ApplicationMonitor monitor) {
	...
	@Override
	public DomainSlot clone() {
		try {
			return (DomainSlot) super.clone();
	 ...
	 * Copy Constructor
	 */
	public DomainSlot createCopy() {
		DomainSlot slot = clone();
		if (this.owner != null) {
			slot.owner = this.owner.clone();
		}

		slot.initialized = this.initialized;
		slot.locked = this.locked;

		slot.value = this.getValue();

		slot.domain = new ArrayList<EObject>();
		if (this.domain != null) {
			slot.domain.addAll(this.domain);
		}

		slot.temporaryDomain = new ArrayList<EObject>();
		if (this.temporaryDomain != null) {
			slot.temporaryDomain.addAll(this.temporaryDomain);
		}
	...
	public boolean instantiate(Variable variable, Map<Variable, DomainSlot> domainMap, EGraph graph,
			List<Map<String, Integer>> seenConfigurations, Map<String, Integer> config) {
		...
		if (!checkedVariables.contains(variable)) {
			...
			for (ContainmentConstraint containmentConstraint : variable.containmentConstraints) {
				...
				if (monitor != null && domainMap.get(containmentConstraint.targetVariable).temporaryDomain != null) {
					this.monitor.addDomainRestrictionRecord(variable.variableId,
							containmentConstraint.targetVariable.variableId,
							domainMap.get(containmentConstraint.getTargetVariable()).temporaryDomain.size());
				}
			}

			// Check the reference constraints:

			List<ReferenceConstraint> referenceConstraints = new ArrayList<ReferenceConstraint>();
			if (!variable.referenceConstraints.isEmpty()) {

				if (constainsVariability(variable.referenceConstraints, new HashMap<String, Integer>(),
						new ArrayList<ReferenceConstraint>())) {
					
					for (ReferenceConstraint constraint : variable.referenceConstraints) {
						if (constraint.getPresenceCondition() == null) {
							if (!checkReferenceConstraint(constraint, null, domainMap)) {
								return false;
							}
						}
					}
					// Make sure to check remaining constraints of current configuration as well.
					referenceConstraints = FeatureConfiguration.INSTANCE.getCurrentConfig().getConstraints();
				} else {
					referenceConstraints = variable.referenceConstraints;
				}

				for (ReferenceConstraint referenceConstraint : referenceConstraints) {
			...
			for (PathConstraint pathConstraint : variable.pathConstraints) {
				...
				if (monitor != null && domainMap.get(pathConstraint.targetVariable).temporaryDomain != null) {
					this.monitor.addDomainRestrictionRecord(variable.variableId,
							pathConstraint.targetVariable.variableId,
							domainMap.get(pathConstraint.targetVariable).temporaryDomain.size());
				}
			}
...
public boolean instantiate(Variable variable, Map<Variable, DomainSlot> domainMap, EGraph graph) {
	...
	public boolean checkReferenceConstraints(List<ReferenceConstraint> constraints, Map<Variable, DomainSlot> domainMap,
			Map<String, Integer> config) {
		// We are here to check the constraints that are part of config
		for (ReferenceConstraint constraint : constraints) {
			...
			if (presenceCondition != null) {
				...
				if (config.equals(featureConfigMap)) {
	...
	public boolean constainsVariability(List<ReferenceConstraint> constraints, Map<String, Integer> configAssumption,
			List<ReferenceConstraint> alreadyVisited) {

		// Have we initialized this already?
		if (FeatureConfiguration.INSTANCE.getConfigurations().isEmpty()) {
			// This could mean, that we have to initialize it first
			FeatureConfiguration.INSTANCE.getConstraintConfigs(constraints, configAssumption, alreadyVisited);

			// If it is still empty, then there are no variants to cover
			if (FeatureConfiguration.INSTANCE.getConfigurations().isEmpty()) {
				return false;
			} else {
				FeatureConfiguration.INSTANCE.getNextConfiguration();
				return true;
			}
		}
	 ...
	 * objects {@link #instantiate(Variable, Map, EGraphImpl)} may be called again.
	 * 
	 * @param sender The variable which uses this domain slot. Only the variable
	 *               which originally initialized this domain slot is able to unlock
	 *               it.
	...
	public boolean unlock(Variable sender) {
		...
		for (int i = refCount + conCount - 1; i >= 0; i--) {
			BinaryConstraint constraint = (i >= refCount) ? sender.containmentConstraints.get(i - refCount)
	 ...
	 * variable that originally initialized this domain slot is able to clear it.
	 * 
	 ...
	 * variable that originally initialized this domain slot is able to reset it.
	 * Resetting means clearing and additionally resetting the temporary domain.
	 * 
	 * @param sender The variable which uses this domain slot.
	 */
	...
	public boolean recheck(Variable variable, Map<Variable, DomainSlot> domainMap) {
		...
		return instantiate(variable, domainMap, null, new ArrayList<Map<String, Integer>>(),
				new HashMap<String, Integer>());
	}

	/**
	 * Checks whether the domain contains additional possible objects that may be
	 * valid for a match.
	 * 
	...
	/**
	 * Locks a specific value for this slot. The slot will also be locked and marked
	 * as initialized and the value can only be changed by calling this method
	 * again.
	 * 
	...
	/**
	 * Initializes the domain slot, i.e., sets the slot's owner and domain under
	 * consideration of the {@link #deterministic} and {@link #injective} flags
	 * 
	...
	public boolean initialize(Variable variable, EGraph graph) {
		...
		// If temporaryDomain is not null, there are BinaryConstraints restricting this
		// slot's domain.
	...
	/**
	 * Tries to set the domainSlot's value and lock the domainSlot. has no effect if
	...
	public boolean checkReferenceConstraint(ReferenceConstraint constraint, BinaryConstraint binaryUserConstraint,
	...
	private boolean mightIncludeMoreVariants = false;

	public boolean mightIncludeMoreVariants() {
		return this.mightIncludeMoreVariants;
	}
