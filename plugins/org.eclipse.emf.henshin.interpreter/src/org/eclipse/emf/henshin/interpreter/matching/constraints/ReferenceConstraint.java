...
public class ReferenceConstraint implements BinaryConstraint {
	...
	// Condition under which this reference is present.
	final Sentence presenceCondition;
	 ...
	 * @param index           Either a constant index (can be <code>null</code>) or
	 *                        a parameter name.
	...
	/**
	 * Constructor which also sets a presence condition under which this constraint can be matched.
	 */
	public ReferenceConstraint(Variable target, EReference reference, Sentence presenceCondition, Object index,
	 ...
	 * check(org.eclipse.emf.henshin.interpreter.matching.constraints.DomainSlot,
	 * org.eclipse.emf.henshin.interpreter.matching.constraints.DomainSlot)
	 */
	@Override
	...
	public boolean check(DomainSlot source, DomainSlot target) {
		...
		if (reference.isMany()) {
			...
			if (targetObjects instanceof EMap && targetVariable.typeConstraint != null
					&& targetVariable.typeConstraint.type != null) {
					...
					if (keyAttribute == attributeConstraint.attribute) {
						...
						try {
							if (attributeConstraint.isConstantValue) {
								...
								} // if not set, we simply fall back to the existing code to restrict the values
									// of the domainslot
							}
							targetObjects = Collections.singletonList((EObject) to);

						} catch (Exception ex) {
							/* fail silently and simply use the complete targetObjects list */ }
						break; // there will be only one "key" attribute constraint, thus we break the loop
								// here.
					}
				}
			}
		...
		if (target.locked) {
			...
			if (!isConstantIndex && !source.conditionHandler.isSet((String) index)) {
				...
				// Try to initialize the parameter with real index. Might fail due to attribute
				// conditions.
				...
				return (calculatedIndex != null) ? targetObjects.indexOf(target.value) == calculatedIndex
	...
	/*
	 * This method resembles the code in V BasicEMap::get(Object key) in order to
	 * get the entry (not only the value) from the map. Unfortunately, the public
	 * interface does not allow getting the entry. However, we need the entry to
	 * properly use the ReferenceConstaint to filter the targetObjects (which is a
	 * set of entries). Thus, we revert back to reflection to call the protected
	 * methods directly which are used in the V BasicEMap::get(Object key) method.
	 */
	...
	private Entry getEntryFromMap(EMap map, Object paramValue)
		...
		if (mapClass == null)
			throw new NoSuchMethodException(
					"Map class does not subclass from BasicEMap which is required to call hashOf, indexOf and entryForKey methods");
	...
	public Sentence getPresenceCondition() {
